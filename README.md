...
- **World → Map geometry → Dark map:** covers the game with solid black so only the SourceSight wireframe, ESP, radar and UI remain visible. **Weapon & hands** restores a responsive, category-specific wireframe viewmodel plus the exact active weapon/grenade/C4 name, icon, ammo and reload state. The toggles work in both Overlay and Full map modes and are saved with the active profile.
...
- **Simple / Advanced setup:** Simple is the default and automatically tunes viewport-dependent line weight, mesh detail, render budget, tracer presentation and radar resolution scaling without enabling gameplay features. The current mode is always visible in the menu header. Advanced exposes the granular rendering and radar controls. Exact minimap range cannot be inferred from collision bounds, so Simple preserves the saved range and Advanced provides manual correction.
- **Player → Bullet trails:** Ion glow, Streak and Minimal styles with fading cores and optional impact markers. Each estimated shot automatically traces across the loaded map and stops at the nearest static-map triangle or bone-based player capsule (including teammates); there is no configurable distance cutoff. Missing bones use a conservative hull. Local shots are included, and detection/rendering run once per frame. A muzzle guard prevents cosmetic offsets from skipping nearby obstacles. New trails pause while matching map geometry is unavailable. These are ammo/aim-based estimates, not server bullet-impact events: spread, recoil correction, penetration, ricochets and dynamic props are not simulated.
