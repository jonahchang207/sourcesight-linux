...
bool MouseAim::SelectNearestEnemy() {
    ...
    // Reference: screen centre in pointer-locked games, else the cursor.
    ...
    // Vision-aware lock: prefer the nearest target that is actually in our
    // line of sight; only fall back to hidden targets when nothing visible is
    // inside the FOV ring.
    int best_index = -1;
    float best_x = 0.0f;
    float best_y = 0.0f;
    for (int pass = 0; pass < 2 && best_index < 0; ++pass) {
        ...
        for (auto& player : cache.players) {
            ...
            // Head position read from memory and projected to screen coords.
            Vec2_t screen;
            if (!PlayerAimScreen(player, matrix, screen_w_, screen_h_, screen))
                continue;
...
void MouseAim::Update() {
    ...
    // Scanner/explicit targets come from the screen itself, so they count as
    // visible; memory targets get the geometric player-occlusion check.
    ...
    // Direct snap: the screen position of the locked head IS the desired
    // cursor position.  The speed cap below turns what would be a teleport
    // into a smooth glide.
    ...
    const float speed_cap = std::max(1.0f, std::max(0.0f, cfg::aim::speed) * dt);
    ...
    // ── deadzone: ignore jitter around the target ───────────────────
    ...
    if (Mouse().Move(static_cast<int>(std::lround(dx)), static_cast<int>(std::lround(dy)))) {
        // Desktop mode: keep the local cursor estimate in sync with the
        // deltas we sent; pointer-locked games hide the cursor, so there the
        // reference stays the screen centre.
        if (!cfg::aim::game_mode) {
